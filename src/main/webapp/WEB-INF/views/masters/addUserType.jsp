...
<c:url value="/checkUniqueUserType" var="checkUniqueUserType"></c:url>
	<!-- Main navbar -->
						...
						<div class="card">
							...
							<div
								...
								<span class="font-size-sm text-uppercase font-weight-semibold"><h6
									...
									style="color: white;" class="card-title">User Type List</a></span>
							</div>
							...
							<div class="card-body">
								...
								<form
									action="${pageContext.request.contextPath}/insertUserType"
									id="submitInsert" method="post">


									<p class="desc text-danger fontsize11">Note : * Fields are
										mandatory.</p>
									<input type="hidden" class="form-control" name="user_type_id" id="user_type_id" value="${userType.userTypeId}">

									<div class="form-group row">
										<label class="col-form-label font-weight-bold col-lg-2"
											for="language_code">User Type<span
											class="text-danger">* </span>:
										</label>
										<div class="col-lg-4">
											<input type="text" class="form-control maxlength-badge-position" name="user_type_name"
												id="user_type_name" maxlength="20" autocomplete="off" onchange="trim(this)" value="${userType.userTypeName}"> 
										<span
												class="validation-invalid-label" id="error_user_type_name"
												style="display: none;">This field is required.</span>	
										<span
												class="validation-invalid-label" id="unq_user_type"
												style="display: none;">User Type Already Exist.</span>
										</div>
										...
										<div class="col-lg-4">
											<input type="text" class="form-control maxlength-badge-position" name="description"
												id="description" maxlength="80" autocomplete="off" onchange="trim(this)" value="${userType.userTypeDesc}">
											<span
												class="validation-invalid-label" id="error_language_name"
												style="display: none;">This field is required.</span>		
										</div>
									</div>
										...
										<div class="col-lg-4">
											<div class="form-check form-check-inline">
												<label class="form-check-label"> <input type="radio"
													...
													${lang.isActive==0 ? 'checked' : ''}> Active
												</label>
											</div>
											...
											<div class="form-check form-check-inline">
												<label class="form-check-label"> <input type="radio"
													...
													id="status_n"
													${lang.isActive==1 ? 'checked' : ''}> In-Active
												</label>
											</div>
										</div>
									</div>
	...
	<script type="text/javascript">
		$(document).ready(function($) {
			...
			$("#submitInsert").submit(function(e) {
				...
				if (!$("#user_type_name").val()) {
					isError = true;
					$("#error_user_type_name").show()
				} else {
					$("#error_user_type_name").hide()
				}
		...
		$("#user_type_name").change(function(){   // 1st
			var userType = $("#user_type_name").val();								
			var uId = $("#user_type_id").val();	
		//	alert(userType)
		
			 $.getJSON('${checkUniqueUserType}', {
				 userType : userType,
				 uId : uId,
				ajax : 'true',
			}, function(data) {
